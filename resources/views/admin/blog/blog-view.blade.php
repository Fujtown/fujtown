        ...
        <div class="col-lg-12">
                                    <div class="card">
                                        <div class="card-body">
                                            ...
                                            <div class="add-btn">
                                            <a href="{{route('coffee.add-blog')}}" class="btn btn-info">Add Blog</a>
                                            </div>
                                            @if(session('success'))
                                        ...
                                        @endif  
                                            ...
                                            @foreach($blogs as $bg)
                                                <tr>
                                                <td>{{ $bg->title }}</td>
                                                <td><img src="{{ asset('storage/' .$bg->image) }}" alt="" class="rounded-circle thumb-sm mr-1"></td>
                                                <td>
                                        <a href="{{route('coffee.edit-blog',$bg->blog_id)}}" class="btn btn-sm btn-success">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <button type="button" class="btn btn-sm btn-danger" onclick="confirmDelete(`{{ $bg->blog_id }}`)">
                                            <i class="fas fa-trash-alt"></i>
                                        </button>
                                        <form id="delete-form-{{ $bg->blog_id }}" action="{{ route('coffee.delete-blog', $bg->blog_id) }}" method="POST" style="display: none;">
                                            @csrf
                                            @method('DELETE')
                                        </form>
